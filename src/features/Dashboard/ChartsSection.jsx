    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {data.map(d => (
        <div key={d.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '10px', color: labelClr, width: '72px', flexShrink: 0 }}>
            {d.label}
          </span>
          <div
            style={{
          ...
          <span style={{ fontSize: '10px', color: valClr, width: '28px', textAlign: 'right' }}>
            {d.value}
          </span>
        </div>
      ))}
...
export default function ChartsSection({ orders }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '10px', marginBottom: '16px' }}>
      <Card title="Orders by status">
        <BarChart data={byStatus} />
      </Card>
      <Card title="Priority split">
        <DonutChart segments={byPriority} total={orders.length || 1} />
      </Card>
    </div>
  );
}